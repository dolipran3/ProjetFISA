...
Anticiper combien d'employés seront présents chaque jour de la semaine est devenu
un enjeu stratégique, tant pour réduire les coûts énergétiques que pour améliorer
...
une \textbf{fenêtre glissante autorégressiive} :
...
\begin{itemize}
  ...
  \item \textbf{PatchTST} \cite{nie2023patchtst} : chaque token = un jour
        de la série endogène seulement (\textit{patch} unitaire).
  \item \textbf{TimeXer} \cite{chen2024timexer} : étend PatchTST par
  ...
  \item \textbf{iTransformer} \cite{liu2024itransformer} : token = variable entière
...
\begin{itemize}
  ...
  \item \textbf{Erreur d'accumulation autorégressiive} : les erreurs se propagent
